...
export async function buildCommandMenuResults(query, context, sessionId) {
  await initialize()
  ...
  let boards = await db.getAll('boards')
  if (sessionId) boards = boards.filter(b => b.sessionId === sessionId)
