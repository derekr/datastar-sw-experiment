import { getDb, beginTx, ALL_STORES, bus, actorId, countRecords, getAllRecords, getRecord } from './db.js'
...
export async function rebuildFromSnapshot() {
  ...
  const snapshot = await getRecord('meta', 'snapshot')
  ...
  // Replay only events after the snapshot seq (exclusive)
  const allEvents = await tx.objectStore('events').getAll()
  const newEvents = allEvents.filter(e => e.seq > snapshot.seq)
...
export async function migrateFromV1() {
  const db = await getDb()
  if ((await countRecords('events')) > 0) return
...
export async function seed() {
  ...
  if ((await countRecords('events')) > 0) return
  if ((await countRecords('columns')) > 0) return
...
export async function initialize() {
  ...
  const db = await getDb()
  const stored = await getRecord('meta', 'actorId')
  if (stored) {
