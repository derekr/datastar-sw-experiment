...
export async function loadTimeTravelEvents(boardId) {
  ...
  // Upcast all events once upfront
  const upcastedEvents = allEvents.map(e => upcast(e))
  // Replay all events, tracking which indices affect this board
  const memTx = createMemoryTx()
  const boardEvents = []
  for (let i = 0; i < upcastedEvents.length; i++) {
    const event = upcastedEvents[i]
  ...
  return { allEvents: upcastedEvents, boardEvents }
}
